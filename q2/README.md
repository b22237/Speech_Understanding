...
3. **Data**: The project uses the **LibriSpeech** dataset. The `train.py` script automatically downloads the `train-clean-100` split (~6.3GB, 251 speakers) for training. Evaluation is performed on the completely unseen `dev-clean` split (40 speakers) to ensure valid generalization metrics.
...
### 2. Trial Generation
Before evaluation, generate the verification trials dev-clean downloaded data to serve as the unseen test set:
*(A helper script generate_test.py to generate `test.txt` is included in the project notes).*

### 3. Evaluation
Evaluate the generated checkpoints:
```bash
python eval.py --checkpoint results/best_model_proposed_fixed_lambdas_seed42.pt --trials data/test.txt --data_dir ./data/LibriSpeech/dev-clean
```

## Results Summary (Seed 42)
| Model | EER (%) | minDCF |
|-------|---------|--------|
| Baseline (Fixed $\lambda$) | 4.000 | 0.00510 |
| Improved (Dynamic $\lambda$) | 3.800 | 0.00710 |
